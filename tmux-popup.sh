...
# Check if the current seesion name starts with "floating", if so detach the session
if [[ "$current_session_name" == floating_* ]]; then
  tmux detach -s "$current_session_name"
  exit 0
fi
